...
def save_upload_image(fileObj):
    ...
    name,ext = filename.split('.') #extract filename and extension
    
    save_filename = 'upload.'+ ext #new name given for saving
    upload_image_path = settings.join_path(settings.SAVE_DIR,save_filename) #call join_path
    
    fileObj.save(upload_image_path) #save the image to path created
...
def array_to_json_format(numpy_array):
    ...
    for pt in numpy_array.tolist():
        points.append({'x':pt[0],'y':pt[1]}) #set x and y coordinates
...
class DocumentScan():
    ...
    def document_scanner(self,image_path):
        ...
        try:
            detail = cv2.detailEnhance(img_re,sigma_s = 20, sigma_r = 0.15) #sigma_s refers to standard deviation, sigma_r refers to radius
            gray = cv2.cvtColor(detail,cv2.COLOR_BGR2GRAY) # GRAYSCALE IMAGE
            blur = cv2.GaussianBlur(gray,(5,5),0)
            # edge detect
            edge_image = cv2.Canny(blur,75,200)

            #enhance lines and close dots/images by morphological transform
    ...
    def calibrate_to_orignal_size(self,four_points):
        ...
        wrap_image = four_point_transform(self.image,four_points_orig) #cropped image
        
        #apply magic colour to wrap image
