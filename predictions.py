...
def parser(text,label):
    if label == 'PHONE':
        ...
        text = text.lower()
        allow_special_char = '@_.\-' #backslash before '-' as it specifies its not a range operator then
        ...
        text = text.title() #make words start with capital
...
def getPredictions(image):
    ...
    df_clean = df.query('text != "" ') #text shouldnt be empty spaces
    content = " ".join([w for w in df_clean['text']]) #combine all text with space
    print(content)
    ...
    datafram_tokens['token'] = datafram_tokens[['start','end']].apply(
        lambda x:doc_text[x[0]:x[1]] , axis = 1) #slice through the start and end positions given
    ...
    img_tagging['label']=img_tagging['label'].str[0]
